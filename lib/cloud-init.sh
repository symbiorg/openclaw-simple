...
write_files:
  ...
  - path: /opt/openclaw/workspace/TOOLS.md
    ...
    content: |
      ...
      ### How to use
      - Check \`workspace/compound/learnings/LEARNINGS.md\` for accumulated insights
      - Check \`workspace/compound/learnings/YYYY-MM-DD.md\` for daily reports
  ...
  - path: /opt/openclaw/.openclaw/skills/compound-engineering/SKILL.md
    ...
    content: |
      ...
      quick references), update relevant \`CLAUDE.md\` files.
      **NEVER modify safety rules or Core Guardrails sections.**

      ### Commit & push
      Stage all changed files, commit with message
      \`compound: nightly report YYYY-MM-DD\`, and push.
